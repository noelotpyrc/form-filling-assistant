...
| **Responder / SHIP MODEL** | **`mergedall` (one merged extractor+responder model), adopted 2026-08-06.** Chapter-1 arc: rounds 2a→2c tuned a split responder (`s2cresp`, frozen 220/223); the polish batch then decided doc-20's one-vs-two question empirically — the union-corpus merged model **beat the frozen extractor on eval v3 (370/414 vs 363, value 97.4 vs 96.5, wrong-field 0%)** while holding responder parity (427/447 vs split's 429), invention 0.0% on the 576-call stress sweep, m3d probe 26/26 sessions / 135/138 / p95 2.25s. One artifact, one server: `qwen35-08b-v2-mergedall-mlx` on :8106, both predictors (serve.py defaults). Upsampling experiment (`mergedup2`) and the data-mix lesson: doc-22 §6. Known residue: dormant caveat 6/13 (gentler treatment on backlog); one probe boolean invention → boolean-provenance guard queued (M4_PLAN). Prior split models (`r3-oracle`+`s2cresp`/`s2dresp`) kept as reference. → doc-20, doc-22, M4_PLAN. |
| **Multi-turn behaviour** | **M3c all-student probe (2026-08-05, sonnet-U): 26/26 sessions to the submit gate, 163/164 assertions** (`probe_runs/m3c_allstudent_sonnetU/`). The one miss is the `conditional_consistency` assertion firing on a volunteered value stored in a condition-inactive field (`english_test_score` before `toefl_required`) — ruled **acceptable-by-design 2026-08-05**: the harness stores supported values; conditions govern use/display. Refine that assertion next probe iteration. All-local latency p50 2.1s / p95 2.4s / max 2.7s per turn — vs the M3b hybrid's p50 3.8 / p95 25.9 (remote responder + retries). U-model caveat: probes with scripted directives need **sonnet-U** (haiku-U ignored the save direction; a haiku run of the same stack sits at `m3c_allstudent/` for reference). Farming stays haiku-U. Probe reports now record `sim_user_model`. |
