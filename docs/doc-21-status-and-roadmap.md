...
| **Responder** | **Trained: `s2cresp` (slice-2c), 2026-08-04 — chapter 1 complete, ship approved.** Three rounds on leon-work: 2a (farm-only targets, failed terminal/submit turns), 2b (+submit-flow injections: frozen 221/223), 2c (+full scenario coverage per doc-22: chitchat/clarify/fix/save-draft/off-form). Final: frozen 223 → 220/223 with ZERO directive failures; five probes 206/211; every Tier-1 family at or above teacher level. Corpus 533/101 rows, all veto-curated. Serve `qwen35-08b-v2-s2cresp-mlx` on :8104 (eos config carries the `<|im_end|>` fix). Remaining residue: 7 one-to-three-token verbosity nicks + 1 muddled save turn (hand-read, accepted). → doc-20, doc-22, M4_PLAN. |
