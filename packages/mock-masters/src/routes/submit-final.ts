...
router.post('/submit-final', authMiddleware, (req, res) => {
  ...
  if (!body.draft_id) {
    res.status(400).json({
      error: {
        code: ErrorCode.INCOMPLETE_SUBMISSION,
        message: 'draft_id is required. Please submit a draft first.',
      },
    });
    return;
  }
  ...
  if (!draft) {
    res.status(404).json({
      error: {
        code: ErrorCode.FORM_NOT_FOUND,
        message: `Draft not found: ${body.draft_id}. Please submit a draft first.`,
      },
    });
    return;
  }
  ...
  if (draft.form_id !== body.form_id) {
    res.status(400).json({
      error: {
        code: ErrorCode.FORM_NOT_FOUND,
        message: 'draft_id does not match form_id.',
      },
    });
    return;
  }
  ...
  const refNumber = `APP-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 99999)).padStart(5, '0')}`;

  const response: SubmitFinalResponse = {
    submission_id: submissionId,
    status: 'submitted',
    confirmation: {
      message: 'Your application to Northfield University has been submitted successfully.',
      reference_number: refNumber,
      next_steps: [
        'You will receive a confirmation email within 24 hours.',
        'Application review takes 4-6 weeks.',
        `You can check status at https://apply.northfield.edu/status/${refNumber}`,
        'Recommendation letter requests will be sent to your listed recommenders.',
      ],
    },
  };
