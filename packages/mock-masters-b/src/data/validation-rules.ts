...
import { mastersBSchema } from './schema.js';
 ...
 */
export function getFieldValue(fieldId: string, allData: Record<string, unknown>): unknown {
...
export function validateField(
  ...
  switch (field.type) {
    ...
    case 'number': {
      ...
      if (isNaN(numVal)) {
        return { field_id: fieldId, valid: false, error: `${field.label} must be a valid number.` };
      }
      if (field.min !== undefined && numVal < field.min) {
        return { field_id: fieldId, valid: false, error: `${field.label} must be at least ${field.min}.` };
      }
      if (field.max !== undefined && numVal > field.max) {
        return { field_id: fieldId, valid: false, error: `${field.label} must be at most ${field.max}.` };
      }
      break;
    }
    ...
    case 'email': {
      if (!EMAIL_REGEX.test(String(value))) {
        return { field_id: fieldId, valid: false, error: `${field.label} must be a valid email address.` };
      }
      break;
    }
    ...
    case 'phone': {
      if (!PHONE_REGEX.test(String(value))) {
        return { field_id: fieldId, valid: false, error: `${field.label} must include a country code (e.g., +1-555-0123).` };
      }
      break;
    }
    ...
    case 'date': {
      ...
      if (expectMonthOnly) {
        if (!DATE_MONTH_REGEX.test(dateStr)) {
          return { field_id: fieldId, valid: false, error: `${field.label} must be in YYYY-MM format.` };
        }
      } else {
        if (!DATE_FULL_REGEX.test(dateStr) && !DATE_MONTH_REGEX.test(dateStr)) {
          return { field_id: fieldId, valid: false, error: `${field.label} must be in YYYY-MM-DD format.` };
        }
      }
      break;
    }
    ...
    case 'select': {
      if (field.options && field.options.length > 0) {
        ...
        if (!validValues.includes(String(value))) {
          return { field_id: fieldId, valid: false, error: `${field.label} must be one of: ${validValues.join(', ')}.` };
        }
      }
      break;
    }
    ...
    case 'multi_select': {
      if (!Array.isArray(value)) {
        return { field_id: fieldId, valid: false, error: `${field.label} must be an array of selections.` };
      }
      if (field.options && field.options.length > 0) {
        const validValues = field.options.map((o) =>
          typeof o === 'string' ? o : o.value,
        ...
        for (const v of value) {
          if (!validValues.includes(String(v))) {
            return { field_id: fieldId, valid: false, error: `Invalid selection "${v}" for ${field.label}.` };
          }
        }
      }
      if (field.max_selections && value.length > field.max_selections) {
        return { field_id: fieldId, valid: false, error: `${field.label} allows at most ${field.max_selections} selections.` };
      }
      break;
    }
    ...
    case 'boolean': {
      if (typeof value !== 'boolean') {
        return { field_id: fieldId, valid: false, error: `${field.label} must be true or false.` };
      }
      break;
    }
