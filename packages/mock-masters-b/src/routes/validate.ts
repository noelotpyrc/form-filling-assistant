...
router.post('/validate', authMiddleware, (req, res) => {
  ...
  if (!body.fields || !Array.isArray(body.fields)) {
    res.status(400).json({
      error: { code: ErrorCode.VALIDATION_FAILED, message: 'fields must be an array of { field_id, value } objects.' },
    });
    return;
  }
  ...
  for (const f of body.fields) {
    allData[f.field_id] = f.value;
  }

  const results: ValidationResult[] = [];
  for (const f of body.fields) {
    const result = validateField(f.field_id, f.value, allData);
    results.push(result);
  }
